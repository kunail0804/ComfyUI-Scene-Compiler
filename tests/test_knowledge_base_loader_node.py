...
def test_node_metadata() -> None:
    assert KnowledgeBaseLoaderNode.RETURN_TYPES == ("KNOWLEDGE_BASE", "STRING", "STRING", "STRING")
    assert KnowledgeBaseLoaderNode.RETURN_NAMES == ("knowledge_base", "warnings", "errors", "raw")
...
def test_loads_knowledge_base(tmp_path) -> None:
    ...
    kb, warnings, errors, *_ = KnowledgeBaseLoaderNode().run(path)
...
def test_relative_path_resolves_against_package(monkeypatch, tmp_path) -> None:
    ...
    kb, warnings, errors, raw = KnowledgeBaseLoaderNode().run("knowledge_base/")
    assert isinstance(kb, KnowledgeBase)
    assert len(kb) > 100
    assert (warnings, errors) == ("", "")
...
def test_empty_knowledge_base_warns(tmp_path) -> None:
    kb, warnings, errors, raw = KnowledgeBaseLoaderNode().run(str(tmp_path))
    assert len(kb) == 0
